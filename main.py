import pygame
# online python project that taught me a bit of game physics, drawing, arithmetics, python classes and functions.
pygame.init()
...
def handle_collision(ball, left_paddle, right_paddle):
    ...
    if ball.x_vel < 0:
        # IDE changed the first <= from (and) but works fine
...
def main():
    ...
    while run:
        ...
        won = False
        if left_score >= WINN_SCORE:
            win_text = "Player 1 Won!"
            won = True
        elif right_score >= WINN_SCORE:
